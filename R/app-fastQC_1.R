...
ezMethodFastQC_1 <- function(input = NA, output = NA, param = NA,
  ...
  rowNames <- paste0(
    "<a href=", reportDirs, "/fastqc_1_report.html>",
    names(files), "</a>"
  )
  ...
  for (i in 1:nFiles) {
    ...
    href <- paste0(
      reportDirs[i], "/fastqc_1_report.html#M",
      0:(ncol(tbl) - 1)
  ...
  ans4Report[["Fastqc_1 quality measures"]] <- tbl
...
##' @templateVar method ezMethodFastQC_1(input=NA, output=NA, param=NA, htmlFile="00index.html")
...
##' }
EzAppFastqc_1 <-
  setRefClass("EzAppFastqc_1",
    contains = "EzApp",
    methods = list(
      initialize = function() {
        "Initializes the application using its specific defaults."
        runMethod <<- ezMethodFastQC_1
        name <<- "EzAppFastqc_1"
        appDefaults <<- rbind(perLibrary = ezFrame(Type = "logical", DefaultValue = TRUE, Description = "Run FastQC_1 per library or per cell for single cell experiment"))
      }
    )
  )
